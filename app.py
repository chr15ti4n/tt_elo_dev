...
if st.session_state.view_mode == "home":
    ...
    def highlight_current(row):
        return ['background-color: #ffff99' if row['Name'] == current_player else '' for _ in row]
    styler_total = (
        df_total.style
    ...
    def make_styled(df, elo_col):
        ...
        # Apply highlight to current player row and hide index
        styler = (
            df2.style
            .apply(highlight_current, axis=1)
            .hide_index()
...
if st.session_state.view_mode == "home":
    ...
    def highlight_current(row):
        return ['background-color: #ffff99' if row['Name'] == current_player else '' for _ in row]
    styler_total = df_total.style.apply(highlight_current, axis=1)
    html_total = styler_total.set_table_attributes('class="total-table"').to_html(border=0)
    st.subheader("Gesamt Leaderboard")
    st.markdown(
        f"<div class='total-table-container'>{html_total}</div>",
        unsafe_allow_html=True
